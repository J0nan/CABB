...
function submitForm(event) {
    ...
    if (validateForm()) {
        setButtonLoading(document.getElementById("uploadFileButton"));
        estimateGas();
    } else {
        unSetButtonLoading(document.getElementById("uploadFileButton"));
    }
}
...
function estimateGas() {
    ...
    try {
        ...
        const address = '/api/file/estimateGas';
        var status = 500;
        fetch(address, {
            method: 'POST',
            body: form
            ...
            .then(data => {
                if (status == 200) {
                    showConfirmation(data.gas.value);
                } else if(status == 422) {
                    formError(data);
                } else if(status == 413){
                    fileToBig();
...
function showConfirmation(gas) {
    ...
    modelBody += "<p><b>Estimated gas:</b> " + gas + "</p>";
